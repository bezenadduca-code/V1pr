...
local function espMakeDocument(obj)
    if documentESPs[obj] then return end
    ...
    documentESPs[obj] = true
end

local function espDoDocuments(on)
    if on then
        for _, v in ipairs(svc.WS:GetDescendants()) do
            if v:IsA("MeshPart") then espMakeDocument(v) end
        end
    else
        for obj in pairs(documentESPs) do
            local h = obj:FindFirstChild("DocumentESP")
            if h then h:Destroy() end
            documentESPs[obj] = nil
        end
        if _docConn then _docConn:Disconnect(); _docConn = nil end
    end
end

local function espBindDocuments(on)
    if _docConn then _docConn:Disconnect(); _docConn = nil end
    if not on then return end
    _docConn = svc.WS.DescendantAdded:Connect(function(v)
        if not esp.documents then return end
        task.wait(0.1)
        if v:IsA("MeshPart") then espMakeDocument(v) end
    end)
end
...
secESP:Toggle({ Title="Documents",  Type="Checkbox", Flag="espDocuments",  Default=esp.documents,  Callback=function(on) esp.documents=on;  task.spawn(function() espDoDocuments(on) end); espBindDocuments(on) end })
...
task.spawn(function()
    ...
    task.defer(function()
        ...
        -- 🌸 Document ESP
        if docESP.enabled then 
            startDocumentESP() 
        end
        
        -- AB Dot (always create)
        abDotCreate()
    end)
end)
...
task.spawn(function()
    ...
    task.defer(function()
        ...
        if esp.items      then task.spawn(function() espDoItems(true)      end) end
        if esp.buildings  then task.spawn(function() espDoBuildings(true)  end) end
        if esp.documents  then task.spawn(function() espDoDocuments(true); espBindDocuments(true) end) end
        if mset.pizza  then task.spawn(scanPizza)   end
        if mset.zombie then task.spawn(scanZombie)  end
        if mset.puddle then task.spawn(scanPuddles) end
        if music.on        then music.thread = task.spawn(musicMonitor) end
