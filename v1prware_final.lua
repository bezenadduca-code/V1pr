...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabDusekkar = win:Tab({ Title = "Dusekkar", Icon = "zap", Locked = true })
...
do
    ...
    task.spawn(function()
        ...
        dusk_oldNC = hookmetamethod(game, "__namecall", function(self, ...)
            ...
            -- Build aimed CFrame and fire via oldNC to avoid re-entering the hook
            local aimedCF = CFrame.lookAt(myHRP.Position, predPos)
            return dusk_oldNC(re, "UseActorAbility", {
...
do
    ...
    sec_029:Paragraph({
        Title   = "How it works",
        Content = "Hooks the RemoteFunction's GetMousePosition callback. When Nova fires, the server asks the client where the mouse is — we return the nearest survivor's position instead.",
    })
    ...
    local function novaPatch()
        ...
        rf.OnClientInvoke = function(reqName, data, ...)
            if reqName == "GetMousePosition" and nova_enabled then
                ...
                if hrp then
                    return hrp.Position + Vector3.new(0, nova_aimOffset, 0)
                end
            end
