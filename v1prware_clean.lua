...
-- Returns the nearest alive survivor CHARACTER MODEL (not HRP)
local function getNearestSurvivor()
    ...
    for _, model in ipairs(sf:GetChildren()) do
        ...
        if shrp and hum and hum.Health > 0 then
            ...
            if d < bd then bd = d; best = model end -- lock model, not HRP
        end
    end
    return best
end

-- Returns true only if target is still in Survivors folder and alive
local function isTargetValid(model)
    if not model then return false end
    local sf = svc.WS:FindFirstChild("Players") and svc.WS.Players:FindFirstChild("Survivors")
    if not sf or model.Parent ~= sf then return false end
    local hum = model:FindFirstChildOfClass("Humanoid")
...
local function aiKillerLoop()
    ...
    local moveConn = svc.Run.Heartbeat:Connect(function()
        ...
        local targetHRP = lockedTarget:FindFirstChild("HumanoidRootPart"); if not targetHRP then return end
