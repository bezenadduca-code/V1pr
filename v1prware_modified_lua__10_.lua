...
------------------------------------------------------------------------
-- DOCUMENT ESP (Forsaken)
------------------------------------------------------------------------
local secDocESP  = tabVisual:Section({ Title = "Document ESP", Opened = true })
local docESPOn   = false
local docESPObjs = {}
local docESPConn = nil

local function docESPMake(obj)
    if docESPObjs[obj] then return end
    ...
    docESPObjs[obj] = true
    print("DOCUMENT ESP:", obj:GetFullName())
end

local function docESPScan(v)
    if v:IsA("MeshPart") then docESPMake(v) end
end

local function docESPStart()
    for _, v in ipairs(svc.WS:GetDescendants()) do docESPScan(v) end
...
local function docESPStop()
    ...
    for obj in pairs(docESPObjs) do
        pcall(function()
            local h = obj:FindFirstChild("DocumentESP"); if h then h:Destroy() end
            local b = obj:FindFirstChild("DocumentBillboard"); if b then b:Destroy() end
        end)
    end
    docESPObjs = {}
end

secDocESP:Toggle({ Title = "Document ESP", Type = "Checkbox", Flag = "docESPOn", Default = docESPOn,
    Callback = function(on)
        docESPOn = on
        if on then docESPStart() else docESPStop() end
    end
})
secDocESP:Button({ Title = "Rescan", Callback = function()
    if not docESPOn then return end
    docESPStop(); docESPStart()
end })
...
task.spawn(function()
    ...
    task.defer(function()
        ...
        if docESPOn    then task.spawn(docESPStart) end
